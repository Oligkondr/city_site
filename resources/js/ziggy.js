const Ziggy = {
    'url': 'http:\/\/localhost:60000', 'port': 60000, 'defaults': {}, 'routes': {
        'sanctum.csrf-cookie': { 'uri': 'sanctum\/csrf-cookie', 'methods': ['GET', 'HEAD'] },
        'moonshine.login': { 'uri': 'admin\/login', 'methods': ['GET', 'HEAD'] },
        'moonshine.authenticate': { 'uri': 'admin\/authenticate', 'methods': ['POST'] },
        'moonshine.logout': { 'uri': 'admin\/logout', 'methods': ['GET', 'HEAD'] },
        'moonshine.profile.store': { 'uri': 'admin\/profile', 'methods': ['POST'] },
        'moonshine.index': { 'uri': 'admin', 'methods': ['GET', 'HEAD'] },
        'moonshine.update-field.through-column': {
            'uri': 'admin\/update-field\/column\/{resourceUri}\/{resourceItem}',
            'methods': ['PUT'],
            'parameters': ['resourceUri', 'resourceItem'],
        },
        'moonshine.update-field.through-relation': {
            'uri': 'admin\/update-field\/relation\/{resourceUri}\/{pageUri}\/{resourceItem}',
            'methods': ['PUT'],
            'parameters': ['resourceUri', 'pageUri', 'resourceItem'],
        },
        'moonshine.async-search': {
            'uri': 'admin\/async-search\/{pageUri}\/{resourceUri?}\/{resourceItem?}',
            'methods': ['GET', 'HEAD'],
            'parameters': ['pageUri', 'resourceUri', 'resourceItem'],
        },
        'moonshine.notifications.readAll': { 'uri': 'admin\/notifications', 'methods': ['GET', 'HEAD'] },
        ...
        'moonshine.component': {
            ...
            'methods': ['GET', 'HEAD'],
            'parameters': ['pageUri', 'resourceUri'],
        },
        'moonshine.method': {
        ...
        'moonshine.page': { 'uri': 'admin\/page\/{pageUri}', 'methods': ['GET', 'HEAD'], 'parameters': ['pageUri'] },
        ...
        'moonshine.handler': {
            ...
            'methods': ['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'],
            'parameters': ['resourceUri', 'handlerUri'],
        },
        ...
        'news.detail': { 'uri': 'news\/{news}', 'methods': ['GET', 'HEAD'], 'parameters': ['news'] },
        'storage.local': {
            'uri': 'storage\/{path}',
            'methods': ['GET', 'HEAD'],
            'wheres': { 'path': '.*' },
